...
# Modified by Wenyu Ouyang <hust2014owen@gmail.com>
#
...
def get_modisfiles(
    ...
    them_urls = []
    for date in dates:
        LOG.info("Looking for the requested files in %s" % date)
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-u", "--username", action="store", dest="username", help="EarthData username"
    )
    parser.add_option(
        "-P", "--password", action="store", dest="password", help="EarthData password"
    )
    parser.add_option(
    ...
    parser.add_option(
        "-y", "--year", action="store", dest="year", type=int, help="Year of interest"
    )
    parser.add_option(
        "-o",
        "--output",
